...
public:    
    ...
    cyg_count32 get_mutex_count() { return mutex_count; };
    Cyg_Mutex  *get_mutex_list() { return mutex_list; };

    // Keep track of the mutex we're waiting on.
    void wait_mutex( Cyg_Mutex *mutex ) { mutex_waiting = mutex; };
    void unwait_mutex() { mutex_waiting = NULL; };
    Cyg_Mutex  *get_mutex_waiting() { return mutex_waiting; };

    // Allow marking threads for diagnostic purposes;
    void     mark()   { mark_flag = true; };
    void     unmark() { mark_flag = false; };
    cyg_bool marked() { return mark_flag; };
