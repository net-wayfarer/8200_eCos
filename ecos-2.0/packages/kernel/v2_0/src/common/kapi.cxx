...
{
   Cyg_Scheduler::register_context_switch_callout( (cyg_scheduler_context_switch_callout)  new_callout, 
...
externC cyg_bool cyg_thread_set_affinity( cyg_handle_t thread, cyg_uint32 affinity_mask )
{
#ifdef CYGPKG_KERNEL_SMP_SUPPORT
    return ((Cyg_Thread *)thread)->set_affinity( affinity_mask );
#else
    return true;
#endif
}

externC cyg_uint32 cyg_thread_get_affinity( cyg_handle_t thread )
{
#ifdef CYGPKG_KERNEL_SMP_SUPPORT
    return ((Cyg_Thread *)thread)->get_affinity();
#else
    return 0xffffffff;
#endif
}
...
externC cyg_count32 cyg_thread_get_mutex_count( cyg_handle_t thread )
{
    return ((Cyg_Thread *)thread)->get_mutex_count();
}
externC cyg_mutex_t *cyg_thread_get_mutex_list( cyg_handle_t thread )
{
    return (cyg_mutex_t*) ((Cyg_Thread *)thread)->get_mutex_list();
}
externC cyg_mutex_t *cyg_thread_get_mutex_waiting( cyg_handle_t thread )
{
    return (cyg_mutex_t*) ((Cyg_Thread *)thread)->get_mutex_waiting();
}
externC void cyg_thread_mark( cyg_handle_t thread )
{
    ((Cyg_Thread *)thread)->mark();
}
externC void cyg_thread_unmark( cyg_handle_t thread )
{
    ((Cyg_Thread *)thread)->unmark();
}
externC bool cyg_thread_marked( cyg_handle_t thread )
{
    return ((Cyg_Thread *)thread)->marked();
}
#endif
...
#else // Broadcom mod: If "thread" isn't totally bogus, then the "id" is no help.
    if( info != NULL )
#endif
    {
        CYG_ASSERT_CLASSC( thread );
...
#if 0
        info->id = id;
#else // Broadcom mod: Don't assume passed "id" is valid.
        info->id = thread->get_unique_id();
#endif
...
#endif
#if 0
#else // Broadcom mod: Pass out the current stack.
        info->current_stack = thread->get_stack_ptr();
#endif
    }
    else result = false;
