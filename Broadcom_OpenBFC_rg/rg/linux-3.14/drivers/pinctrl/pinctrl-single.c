...
static void pcs_pin_info_show(struct pinctrl_dev *pctldev,
					struct seq_file *s,
					unsigned pin)
{
	struct pcs_device *pcs;
...
static const struct pinctrl_ops pcs_pinctrl_ops = {
	...
	.pin_dbg_show = pcs_pin_info_show,
	...
	unsigned group)
{
	...
	dev_info(pcs->dev, "enabling %s function%i\n",
		func->name, fselector);
					...
					unsigned group)
{
	...
	if (pcs->foff == PCS_OFF_DISABLED) {
		dev_info(pcs->dev, "ignoring disable for %s function%i\n",
			func->name, fselector);
		return;
	}

	dev_info(pcs->dev, "disabling function%i %s\n",
		fselector, func->name);
...
static void pcs_pinconf_info_show(struct pinctrl_dev *pctldev,
				struct seq_file *s, unsigned pin)
{
}

static void pcs_pinconf_group_info_show(struct pinctrl_dev *pctldev,
				struct seq_file *s, unsigned selector)
{
}

static void pcs_pinconf_config_info_show(struct pinctrl_dev *pctldev,
					struct seq_file *s,
					unsigned long config)
{
...
static const struct pinconf_ops pcs_pinconf_ops = {
	...
	.pin_config_dbg_show = pcs_pinconf_info_show,
	.pin_config_group_dbg_show = pcs_pinconf_group_info_show,
	.pin_config_config_dbg_show = pcs_pinconf_config_info_show,
	.is_generic = true,
};
...
static int pcs_allocate_pin_table(struct pcs_device *pcs)
{
	...
	dev_info(pcs->dev, "allocating %i pins\n", nr_pins);
			  ...
			  struct pcs_conf_vals **conf, unsigned long **settings)
{
	...
	if (ret < 0)
		dev_info(pcs->dev, "failed to match enable or disable bits\n");
