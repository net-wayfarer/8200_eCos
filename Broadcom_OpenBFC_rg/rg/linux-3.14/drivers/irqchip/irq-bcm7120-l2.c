...
static void bcm7120_l2_intc_irq_handle(unsigned int irq, struct irq_desc *desc)
{
	...
	for (idx = 0; idx < b->n_words; idx++) {
		...
		/* Mask status with the per-L1 interrupt map_mask */
		pending &= data->irq_map_mask[idx];
		irq_gc_unlock(gc);
				    ...
				    u32 *valid_mask)
{
	...
	for (idx = 0; idx < data->n_words; idx++) {
		 ...
		 */
		l1_data->irq_map_mask[idx] = be32_to_cpup(map_mask + irq * data->n_words + idx);
					...
					struct device_node *parent)
{
	...
	u32 valid_mask[MAX_WORDS] = { [0 ... (MAX_WORDS - 1)] = 0 };
