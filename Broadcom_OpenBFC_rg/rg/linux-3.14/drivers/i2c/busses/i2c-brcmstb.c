...
/*_ Turtle clocks and slow cpus require more time to cross the road/bus.
    In order to handle the 50kHz rate on some CPU's we need to bump the
	time from 100ms to 1s                                              */
#define I2C_TIMEOUT					1000 /* msecs */
...
#ifdef CONFIG_I2C_HW_ATOMIC_SPINLOCK

#define I2C_SPINLOCK_TIMEOUT     1000

/* Mbox lock bit7, bus type-bit5 GISB/UBUS=0 (UBUS)
   bit0:4-UBUS source ID for Viper=0               */
#define I2C_SPINLOCK_DOCSIS_OWNER 0x80

/* Mbox lock bit7, bus type-bit5 GISB/UBUS=1 (GISB)
   bit0:4-UBUS source ID n/a for Zephyr            */
#define I2C_SPINLOCK_RG_OWNER     0xa0

/* No Current Owner                                */
#define I2C_SPINLOCK_NO_OWNER     0x00
#endif
...
struct brcmstb_i2c_dev {
	...
	void __iomem *hw_atomic_spinlock;
#endif
};
...
#ifdef I2C_DEBUG
#define hex_dump_pkt(data, len) \
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_NONE, 16, 1, \
		       data, len, true)
#endif

/* Add in atomic spinlock support */
#ifdef CONFIG_I2C_HW_ATOMIC_SPINLOCK
static void bcm_i2c_spinlock(struct brcmstb_i2c_dev *dev)
{
	unsigned i = 0;
	u32 val;

	mutex_lock(&dev->sw_lock);
	__raw_writel(I2C_SPINLOCK_RG_OWNER, dev->hw_atomic_spinlock);
	val = __raw_readl(dev->hw_atomic_spinlock);
	while (val != I2C_SPINLOCK_RG_OWNER) {
		if (i++ > I2C_SPINLOCK_TIMEOUT) {
...
static void bcm_i2c_spinunlock(struct brcmstb_i2c_dev *dev)
{
	__raw_writel(I2C_SPINLOCK_NO_OWNER, dev->hw_atomic_spinlock);
	mutex_unlock(&dev->sw_lock);
}

static int bcm_i2c_check_spinlock(struct brcmstb_i2c_dev *dev)
{
	u32 val = __raw_readl(dev->hw_atomic_spinlock);
	if (val != I2C_SPINLOCK_RG_OWNER) {
...
static int brcmstb_i2c_wait_if_busy(struct brcmstb_i2c_dev *dev)
{
	...
	while ((bsc_readl(dev, iic_enable) & BSC_IIC_EN_INTRP_MASK)) {
		if (time_after(jiffies, timeout)) {
            dev_dbg(dev->device, "brcmstb_i2c_wait_if_busy timeout\n");
			return -ETIMEDOUT;
		}
		cpu_relax();
	}
...
static int brcmstb_i2c_wait_for_completion(struct brcmstb_i2c_dev *dev)
{
	...
	if (dev->irq >= 0) {
		if (!wait_for_completion_timeout(&dev->done, timeout)) {
            dev_dbg(dev->device, "brcmstb_i2c_wait_for_completion int failure\n");
			ret = -ETIMEDOUT;
        }
	} else {
		...
		do {
			...
			if (time_after(jiffies, time_left)) {
				ret = -ETIMEDOUT;
                dev_dbg(dev->device, "brcmstb_i2c_wait_for_completion polling failure\n");
				break;
			}
			cpu_relax();
		} while (!bsc_intrp);
				...
				enum bsc_xfer_cmd cmd)
{
	...
	if (rc < 0) {
        dev_dbg(dev->device, "brcmstb_i2c_wait_if_busy failure\n");
		return rc;
	}
			       ...
			       struct i2c_msg *msg)
{
	...
	if (msg->flags & I2C_M_TEN) {
		...
		/*_ Did the boat pull into port?  If so he has a separate parking spot. */
		if (dev->port_select) {
			bsc_writel(dev, (unsigned long)(addr+0x80000000), chip_address);
		}
		else {
			bsc_writel(dev, addr, chip_address);
		}
			    ...
			    struct i2c_msg msgs[], int num)
{
...
#ifdef CONFIG_I2C_HW_ATOMIC_SPINLOCK
	/*_ Keymaker, lock the door. */
	bcm_i2c_spinlock(dev);
#endif
	...
	for (i = 0; i < num; i++) {
...
#ifdef I2C_DEBUG
		printk("========== DUMP BEFORE ========\n");
		hex_dump_pkt( ioremap( 0x14e00e00, 84 ), 84);
#endif
		...
		if (!(pmsg->flags & I2C_M_NOSTART)) {
			dev_dbg(dev->device, "brcmstb_i2c_xfer slave address:0x%x\n", pmsg->addr);
		...
		while (len) {
			...
			rc = brcmstb_i2c_xfer_bsc_data(dev, tmp_buf,
						       bytes_to_xfer, pmsg);
			if (rc < 0) {
                dev_info(dev->device,"brcmstb_i2c_xfer_bsc_data failed.\n");
				goto out;
            }
...
out:
#ifdef CONFIG_I2C_HW_ATOMIC_SPINLOCK
	/*_ Keymaker, unlock the door. */
	bcm_i2c_spinunlock(dev);
#endif
...
static int brcmstb_i2c_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "I2C driver probe entry.");
...
#ifdef CONFIG_I2C_HW_ATOMIC_SPINLOCK
	/* Map in the spinlock address */
	iomem = platform_get_resource(pdev, IORESOURCE_MEM, 1);
	dev->hw_atomic_spinlock = devm_ioremap_resource(dev->device, iomem);
	if (IS_ERR(dev->hw_atomic_spinlock))
		return PTR_ERR(dev->hw_atomic_spinlock);
#endif
    ...
    if (of_property_read_u32(dev->device->of_node,
                 "port-select", &dev->port_select)) {
        dev_warn(dev->device, "using default port select:%d\n",0);
        dev->port_select = 0;
    }
	...
	dev_info(dev->device, "%s@%dhz registered in %s mode using port select:%d\n",
		 int_name ? int_name : " ", dev->clk_freq_hz,
		 (dev->irq >= 0) ? "interrupt" : "polling", dev->port_select);
