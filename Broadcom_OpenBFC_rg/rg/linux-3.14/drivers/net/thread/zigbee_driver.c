...
#define ZIGBEE_MAJOR 106 /* use this for our major device number */
static const char zigbee_name[] = "broadot";
...
*/
                {
                    ...
                    size_in_words = 32; // the format isn't compatible with BroadBee
    ...
    if (status & BCHP_RF4CE_CPU_HOST_STB_L2_CPU_STATUS0_MBOX_Z2H_FULL_INTR_MASK) { /* We have received a message from the Zigbee */
        ...
        if (BDEV_RD(BCHP_RF4CE_CPU_CTRL_Z2H_MBOX_FIFO_DEPTH) != 0) {
            ...
            BDEV_SET(BCHP_RF4CE_CPU_HOST_STB_L2_CPU_CLEAR0, BCHP_RF4CE_CPU_HOST_STB_L2_CPU_CLEAR0_MBOX_Z2H_FULL_INTR_MASK); // added by  huajun
        ...
        //BDEV_SET(BCHP_RF4CE_CPU_HOST_STB_L2_CPU_CLEAR0, BCHP_RF4CE_CPU_HOST_STB_L2_CPU_CLEAR0_MBOX_Z2H_FULL_INTR_MASK);  // deleted by huajun
...
int brcmstb_zigbee_probe(struct platform_device *pdev)
{
	...
	trusted = of_get_property(of_node, "trust-status", NULL);
	if (!trusted | (strncmp(trusted, "true", 4) != 0)) {
		/* Deal with old BOLT versions: If "trust-status" property is
		 * not found or not "true", BOLT has incorrectly enabled this
		 * node on unsupported HW. In this case, we need to bail out. */
		printk(KERN_ALERT "Encountered incorrect rf4ce node status\n");
		return -ENODEV;
	}
