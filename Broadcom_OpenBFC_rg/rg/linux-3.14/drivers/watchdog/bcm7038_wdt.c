...
static void bcm7038_wdt_set_timeout_reg(struct watchdog_device *wdog)
{
	...
	__raw_writel(timeout, wdt->reg + WDT_TIMEOUT_REG);
}
...
static int bcm7038_wdt_ping(struct watchdog_device *wdog)
{
	...
	__raw_writel(WDT_START_1, wdt->reg + WDT_CMD_REG);
	__raw_writel(WDT_START_2, wdt->reg + WDT_CMD_REG);
...
static int bcm7038_wdt_stop(struct watchdog_device *wdog)
{
	...
	__raw_writel(WDT_STOP_1, wdt->reg + WDT_CMD_REG);
	__raw_writel(WDT_STOP_2, wdt->reg + WDT_CMD_REG);
...
static unsigned int bcm7038_wdt_get_timeleft(struct watchdog_device *wdog)
{
	...
	time_left = __raw_readl(wdt->reg + WDT_CMD_REG);
