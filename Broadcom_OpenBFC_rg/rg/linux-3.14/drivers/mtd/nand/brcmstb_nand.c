...
static irqreturn_t brcmstb_nand_ctlrdy_irq(int irq, void *data)
{
	...
	if ((ctrl->irq_cascaded) && (ctrl->dma_pending))
#else
	if (ctrl->dma_pending)
#endif
		return IRQ_HANDLED;
...
static irqreturn_t brcmstb_nand_irq(int irq, void *data)
{
	...
	irqreturn_t rv_flash = IRQ_NONE, rv_ctrl = IRQ_NONE;

	if (has_flash_dma(ctrl) && flash_dma_irq_done(ctrl))
		rv_flash = brcmstb_nand_dma_irq(irq, data);

	if (HIF_TEST_IRQ(NAND_CTLRDY)) {
		HIF_ACK_IRQ(NAND_CTLRDY);
		rv_ctrl =  brcmstb_nand_ctlrdy_irq(irq, data);
	}

	return (rv_flash | rv_ctrl);
}
#else
static irqreturn_t brcmstb_nand_irq(int irq, void *data)
{
...
static int bcmnand_ctrl_busy_poll(const struct device *dev)
{
	unsigned long timeout = jiffies + msecs_to_jiffies(100);

	while (!(BDEV_RD(BCHP_NAND_INTFC_STATUS) &
		 BIT(BCHP_NAND_INTFC_STATUS_CTLR_READY_SHIFT))) {
		if (time_after(jiffies, timeout)) {
			dev_warn(dev, "timeout on ctrl_ready\n");
			return -ETIMEDOUT;
		}
		cpu_relax();
	}
...
static void brcmstb_nand_send_cmd(struct brcmstb_nand_host *host, int cmd)
{
	...
	BUG_ON(bcmnand_ctrl_busy_poll(&host->pdev->dev) == -ETIMEDOUT);
