...
#include <linux/brcmstb/pcie_api.h>
...
#define BRCM_MSI_TARGET_ADDR_LO		0xfffffffc
#define BRCM_MSI_TARGET_ADDR_HI		0x0
...
extern bool pci_bus_read_dev_vendor_id(struct pci_bus *bus, int devfn, u32 *pl,
                              int crs_timeout);
...
static int brcm_save_pci_dev_state(struct pci_dev *dev, void *ctx)
{
	return pci_save_state(dev);
}

static int brcm_restore_pci_dev_state(struct pci_dev *dev, void *ctx)
{
	u32 l;

	/* Since the device may have been powered off, we have to account
	* for the possibility of "Configuration Retry Status".
	*/
	if (pci_bus_read_dev_vendor_id(dev->bus, dev->devfn, &l, 1000)) {
		pci_restore_state(dev);
	} else {
		pr_err("could not restore state of pci dev on bus %d\n",
		dev->bus->number);
		return -1;
	}
	return 0;
...
static void brcm_pcie_setup_early(struct brcm_pcie *pcie)
{
	...
	__raw_writel(0x00000010, base + PCIE_MISC_RC_BAR2_CONFIG_LO);
...
static void enter_l23(struct brcm_pcie *pcie)
{
	...
	if (!timeout)
		dev_err(pcie->dev, "Unable to enter L23\n");
}
...
static void pcie_resume_one(struct brcm_pcie *pcie)
{
	...
	brcm_setup_pcie_bridge(pcie->nr, pcie->sys);

	pcie->suspended = false;
	atomic_inc(&pcie->use_count);
}
				...
				struct regulator_cntrl_info *reg_cntrl_info)
{
	...
	list_for_each(pos, &pcie->pwr_supplies) {
		supply = list_entry(pos, struct brcm_dev_pwr_supply,
							node);
		/* Update Name */
		strncpy(reg_cntrl_info[supply_cnt].name,
				supply->name, sizeof(supply->name));

		/* Update Regulator State */
		reg_cntrl_info[supply_cnt].regulator_state =
			regulator_is_enabled(supply->regulator);
		supply_cnt++;
	}
	return 0;
}
EXPORT_SYMBOL(brcm_pcie_regulator_info);
/*
* Notifier support to dynamically turn on/off the regulators of a given
* PCIE device (endpoint)
*/

static int brcm_pcie_bus_notifier(struct notifier_block *nb,
                                 unsigned long event, void *__dev)
{
	...
	switch (event) {
	/* driver about to be bound */
	case BUS_NOTIFY_BIND_DRIVER:
		/* Only do this if we had a prior unbind call which drops
		 * refcount to zero
		 */
		if (atomic_read(&pcie->use_count) == 0) {
			pcie_resume_one(pcie);
			pci_walk_bus(pcie->bus, brcm_restore_pci_dev_state, NULL);
		}
		break;
	/* driver unbound from device */
	case BUS_NOTIFY_UNBOUND_DRIVER:
		/* Only do this if we had an initial use count of one */
		if (atomic_read(&pcie->use_count) == 1) {
			pci_walk_bus(pcie->bus, brcm_save_pci_dev_state, NULL);
			pcie_suspend_one(pcie);
		}
		break;
	default:
...
static struct notifier_block brcm_pcie_nb = {
       .notifier_call = brcm_pcie_bus_notifier,
};
...
brcm_pcibios_fixup(struct pci_dev *dev)
{
	...
	if (pci_is_root_bus(dev->bus)) {
		...
		/* Associate this device with the RC */
		pcie->bus = dev->bus;
	}
	dev_info(pcie->dev,
...
int __init brcm_pcibios_init(void)
{
	...
	bus_register_notifier(&pci_bus_type, &brcm_pcie_nb);
