...
/* BEGIN: PTI add for cpu_architecture */
...
static int
_buzzz_procfs_log_show(struct file *filp, char __user *buffer, size_t length, loff_t *data);

static const struct file_operations buzzz_procfs_func_fops = {
	.owner          = THIS_MODULE,
	.read           = _buzzz_procfs_log_show
};
...
/* END: PTI add for cpu_architecture */
...
#define BUZZZ_PRINT(_fmt, _args...) \
	printk(CLRg "BUZZZ %s: " _fmt CLRnl, __func__, ##_args)
...
#define BUZZZ_PRERR(_fmt, _args...) \
	printk(CLRerr "BUZZZ " BUZZZ_VER_FMTS " %s ERROR: " _fmt CLRnl, \
		BUZZZ_VER_FMT(BUZZZ_SYS_VERSION), __func__, ##_args)
...
#define XBUZZZ_SNPRINTF(_fmt, _args...) \
	bytes += printk(_fmt, ##_args)
	       ...
	       uint32_t log_size, buzzz_dump_log_fn_t dump_log_fn)
{
	...
	/* XXX Simplify the following options */
...
_buzzz_symbol(char *p, unsigned long address)
{
	...
	if (eos == (char *)NULL)
		XBUZZZ_SNPRINTF("  %s" CLRnl, symbol_buf);
	else {
		*eos = '\0';
		sscanf(eos+1, "0x%lx", &offset);
		XBUZZZ_SNPRINTF("  %s" CLRnorm, symbol_buf);
		if (offset)
			XBUZZZ_SNPRINTF(" +0x%lx", offset);
		XBUZZZ_SNPRINTF("\n");
	}
	return bytes;
}
...
_buzzz_func_indent(char *p)
{
	...
	if (buzzz_g.priv.func.config_exit) {
		...
		for (indent = 0U; indent < buzzz_g.priv.func.indent; indent++)
			XBUZZZ_SNPRINTF("%s", BUZZZ_FUNC_INDENT_STR);
	}
	return bytes;
}
...
buzzz_func_dump_log(char *p, int bytes, void *l)
{
	...
	if (log->arg0.klog.is_klog) {   /* print using registered log formats */
		...
		switch (log->arg0.klog.args) {
			case 0: XBUZZZ_SNPRINTF(fmt); break;
			case 1: XBUZZZ_SNPRINTF(fmt, log->arg1); break;
			case 2: XBUZZZ_SNPRINTF(fmt, log->arg1, log->arg2); break;
			case 3: XBUZZZ_SNPRINTF(fmt, log->arg1, log->arg2, log->arg3); break;
		}
		...
		if (log->arg0.site.is_ent)
			XBUZZZ_SNPRINTF("%s", CLRr "=>");
		else
			XBUZZZ_SNPRINTF("%s", CLRg "<=");
...
#if defined(BUZZZ_CONFIG_CPU_ARMV7_B15)
/* BUZZZ uses ARMv7 A15 counters #2,#3,#4,#5 as BUZZZ counters #0,#1,#2,#3 */
#define BUZZZ_ARM_CTR_SEL(ctr_idx) ((ctr_idx) + 2)
#define BUZZZ_ARM_CTR_BIT(ctr_idx) (1 << BUZZZ_ARM_CTR_SEL(ctr_idx))
#define BUZZZ_ARM_IDCODE           (0x0F)
#endif  /*  BUZZZ_CONFIG_CPU_ARMV7_A9 */
...
buzzz_kevt_parse_log(char *p, int bytes, buzzz_kevt_log_t *log)
{
	if (log->arg0.klog.id == BUZZZ_KEVT_ID_SCHEDULE) {
		...
		XBUZZZ_SNPRINTF(CLRc "TASK_SWITCH from[%s %u:%u:%u] to[%s %u:%u:%u]\n",
...
buzzz_kevt_dump_log(char *p, int bytes, buzzz_kevt_log_t *log)
{
	...
	if (cpu >= NR_CPUS) {
		XBUZZZ_SNPRINTF(CLRerr "--- skipping log on CPU %u? ---" CLRnl, cpu);
		return bytes;
	}
	...
	for (ctr = 0U; ctr < BUZZZ_PMU_COUNTERS; ctr++) {
		...
		if ((delta[ctr] > 1000000000) && (buzzz_g.priv.kevt.skip == false)) {
			XBUZZZ_SNPRINTF(CLRerr "--- skipping log"
	...
	if (buzzz_g.priv.kevt.pmu_group == BUZZZ_PMU_GROUP_GENERAL) {
		...
		XBUZZZ_SNPRINTF("%s%3u%s %10u %10u %10u %10u %s%5u.%03u%s %s%6u.%03u%s\t",
		...
		XBUZZZ_SNPRINTF("%s%3u%s %10u %10u %10u %10u\t",
			(cpu == 0) ? CLRyk : CLRmk, cpu, CLRnorm,
			delta[0], delta[1], delta[2], delta[3]);
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
static int BUZZZ_NOINSTR_FUNC
_buzzz_procfs_log_show(struct file *fp, char __user *buffer, size_t length, loff_t *data)
#else
static int BUZZZ_NOINSTR_FUNC
...
#endif
{
	...
	*start = p;  /* prepare start of buffer for printing */
	...
	if (buzzz_g.priv.func.log_index > buzzz_g.priv.func.log_count) {
		...
		XBUZZZ_SNPRINTF(CLRbold "BUZZZ_DUMP END" CLRnl);
		...
		if (index == 0U) {  /* Log the header */
			...
			XBUZZZ_SNPRINTF(CLRbold "BUZZZ_DUMP BGN total<%u>" CLRnl,
				buzzz_g.priv.func.log_count);
...
#else
	/* create_proc_read_entry removed in linux 3.10.0, use proc_create_data() instead. */
	 buzzz_log = proc_create_data(BUZZZ_NAME "/func", S_IRUGO, NULL,
			 &buzzz_procfs_func_fops, (void *)&buzzz_g);
#endif
