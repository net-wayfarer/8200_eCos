...
static void update_child_timer(struct net *net,
			       struct nf_conn *ct_master)
{
	unsigned int bucket;
	...
	for (bucket = 0; bucket < net->ct.htable_size; bucket++) {
		n = rcu_dereference(
		   hlist_nulls_first_rcu(&net->ct.hash[bucket]));
		...
		while (!is_a_nulls(n)) {
			...
			if (ct->master == ct_master) {
				mod_timer(&ct->timeout, jiffies + HZ);
			}
next_ct:
			       ...
			       unsigned int cseq)
{
	...
	update_child_timer(&init_net, ct);
	flush_expectations(ct, true);
	return NF_ACCEPT;
}
