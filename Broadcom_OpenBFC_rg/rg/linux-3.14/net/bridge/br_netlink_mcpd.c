...
int mcpd_process_skb(struct net_bridge *br, struct sk_buff *skb, unsigned short protocol)
{
    ...
    len   = skb->len;
    pData = skb->data;
    if (vlan_eth_hdr(skb)->h_vlan_proto != htons(protocol) )
    {
        if ( vlan_eth_hdr(skb)->h_vlan_proto == htons(ETH_P_8021Q) )
        {
            if ( vlan_eth_hdr(skb)->h_vlan_encapsulated_proto == htons(protocol) )
            {
...
struct netlink_kernel_cfg cfg = {
		.groups	= 0,
		.input	= mcpd_nl_rcv_skb,
	};

    printk(KERN_INFO "Initializing MCPD Module\n");

    nl_sk = netlink_kernel_create(&init_net, NETLINK_MCPD, &cfg);
