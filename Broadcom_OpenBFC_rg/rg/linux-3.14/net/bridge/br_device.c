...
struct br_dbevent_notifier __rcu *dbevent_cb;
int br_register_dbevent_notifier(struct br_dbevent_notifier *cb)
{
	rcu_assign_pointer(dbevent_cb,  cb);
	return 0;
}
EXPORT_SYMBOL(br_register_dbevent_notifier);
void br_unregister_dbevent_notifier(struct br_dbevent_notifier *cb)
{
	RCU_INIT_POINTER(dbevent_cb, NULL);
}
EXPORT_SYMBOL(br_unregister_dbevent_notifier);
#endif
