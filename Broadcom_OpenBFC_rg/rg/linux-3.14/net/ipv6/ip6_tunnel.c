...
#ifdef CONFIG_SYSCTL
/* BRCM Change: For allowing IPv6 fragmentation */
static struct addrconf_sysctl_table
{
	struct ctl_table_header *sysctl_header;
	ctl_table addrconf_vars[2];
} addrconf_sysctl __read_mostly = {
	.sysctl_header = NULL,
	.addrconf_vars = {
		{
			.procname	= "local_df",
			.data		= (void *)offsetof(struct ip6_tnl, local_df),
			.maxlen		= sizeof(int),
			.mode		= 0644,
...
static int addrconf_sysctl_register(struct net *net, char *dev_name,
		struct ip6_tnl *idev)
{
	int i;
	...
	for (i = 0; t->addrconf_vars[i].data; i++) {
		t->addrconf_vars[i].data = (void *)((size_t)t->addrconf_vars[i].data
						    + (size_t)idev);
...
static void addrconf_sysctl_unregister(struct ip6_tnl *p)
{
	struct addrconf_sysctl_table *t;
...
#ifdef CONFIG_SYSCTL
	/* BRCM Change: For allowing IPv6 fragmentation */
	addrconf_sysctl_register(net, dev->name, t);
#endif
	return t;
			 ...
			 __u32 *pmtu)
{
	...
	if (skb->len > mtu) {
		if (t->local_df &&
			(ip_may_fragment(skb) || !ip_exceeds_mtu(skb, dev->mtu-sizeof(struct ipv6hdr)))) {
			/* For not allowing IPv6 fragmentation when DF is set and ip exceeds mtu after adding IPv6's header.*/
			ip6frag = 1;
		} else {
		    *pmtu = mtu;
		    err = -EMSGSIZE;
