...
static int dump_autoneg(struct ethtool_cmd *ep)
{
	fprintf(stdout, "%s\n",
...
static int dump_speed(struct ethtool_cmd *ep)
{
	u32 speed;
...
static int dump_phyad(struct ethtool_cmd *ep)
{
	fprintf(stdout, "%d\n", ep->phy_address);
...
static int dump_xcvr(struct ethtool_cmd *ep)
{
...
static int dump_mdix(struct ethtool_cmd *ep)
{
	if (ep->port == PORT_TP) {
...
static int dump_sopass(struct ethtool_wolinfo *wol)
{
	int i;
	int delim = 0;
...
static int dump_msglvl(struct ethtool_value *edata)
{
...
static int dump_lstatus(struct ethtool_value *edata)
{
...
static int do_gautoneg(struct cmd_context *ctx)
{
	int err;
	struct ethtool_cmd ecmd;
...
static int do_gspeed(struct cmd_context *ctx)
{
	int err;
	struct ethtool_cmd ecmd;
...
static int do_gduplex(struct cmd_context *ctx)
{
	int err;
	struct ethtool_cmd ecmd;
...
static int do_get(struct cmd_context *ctx)
{
	struct ethtool_cmd ecmd;
	struct ethtool_wolinfo wolinfo;
	struct ethtool_value edata;
	...
	int port_wanted = 0;
	int mdix_wanted = 0;
	int autoneg_wanted = 0;
	int phyad_wanted = 0;
	int xcvr_wanted = 0;
	int wol_wanted = 0;
	int sopass_wanted = 0;
	int msglvl_wanted = 0;
	int lstatus_wanted = 0;
	int argc = ctx->argc;
	char **argp = ctx->argp;
	int i;
	int err;
	...
	if (speed_wanted || duplex_wanted || port_wanted || xcvr_wanted ||\
		mdix_wanted || autoneg_wanted || phyad_wanted) {
		ecmd.cmd = ETHTOOL_GSET;
		err = send_ioctl(ctx, &ecmd);
		if (err == 0) {
			if (speed_wanted) {
...
static const struct option {
	...
	{ "--get-phy-tunable", 1, do_get_phy_tunable, "Get PHY tunable",
	  "		[ downshift ]\n"},
	{ "--get" , 1, do_get, "Query generic options"},
