...
static const char *watchdog_devname = WATCHDOG1;
...
static int setup_clients(void)
{
    ...
    client_add(fd, process_listener, client_pid_dead);
    return 0;
}
...
static void close_watchdog(void)
{
    ...
    rv = write(dev_fd, "V", 1);
    if (rv < 0)
        log_error("%s disarm write error %d", watchdog_devname, errno);
    else
        log_error("%s disarmed", watchdog_devname);
...
static int setup_watchdog(void)
{
    ...
    watchdog_devname = WATCHDOG1;
    dev_fd = open(watchdog_devname, O_WRONLY | O_CLOEXEC);
    if (dev_fd < 0) {
        watchdog_devname = WATCHDOG0;
        dev_fd = open(watchdog_devname, O_WRONLY | O_CLOEXEC);
        if (dev_fd < 0) {
            log_error("cannot open watchdog device");
            return dev_fd;
        }
    }
    ...
    if (rv < 0) {
        log_error("watchdog device failed to report timeout");
        close_watchdog();
        return -1;
    }
    ...
    if (rv < 0) {
        log_error("watchdog device failed to set timeout");
        close_watchdog();
        return -1;
    }

    if (timeout != fire_timeout) {
        log_error("watchdog device failed to set new timeout");
        close_watchdog();
        return -1;
    }
  out:
    log_error("watchdog device armed with fire_timeout %d", fire_timeout);
...
static void stop_watchdog(void)
{
    ...
    int rv = ioctl(dev_fd, WDIOC_SETOPTIONS, &val);
    log_debug("stop watchdog %d", rv);
}
...
#define NOWAYOUT_FILE "/sys/module/bcm7038_wdt/parameters/nowayout"

int set_driver_nowayout(void)
{
    char buf[2];

    int fd = open(NOWAYOUT_FILE, O_WRONLY);
    if (fd < 0)
        return fd;
    ...
    fd = open(NOWAYOUT_FILE, O_RDONLY);
    if (fd < 0)
        return fd;
...
static int test_loop(int wd_enable)
{
    void (*workfn) (int ci);
    void (*deadfn) (int ci);
    uint64_t test_time;
    ...
    while (1) {
        ...
        if (monotime() - test_time >= test_interval) {
            ...
            fail_count += wdmd_test_tasks();

            pet_watchdog();
            if (fail_count && wd_enable) {
                if (set_driver_nowayout() < 0) {
                    log_error("watchdog device could not set nowayout");
                    system("reboot");
                }
                else {
                    system("halt");
...
static void print_usage_and_exit(int status)
{
    ...
    printf("-F,-f <task file>     file containing processes to be monitored\n");
    exit(status);
}
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        c = getopt_long(argc, argv, "hVDH:G:df:",
        ...
        switch (c) {
            ...
            case 'd':
                ...
                break;
            case 'f':
                rv = wdmd_process_file(optarg);
                if (rv < 0) {
                    log_error("Failure reading task file %s\n", optarg);
                    exit(EXIT_FAILURE);
                }
                log_debug("monitoring %d tasks\n", rv);
                break;
        }
    }
    ...
    if (wd_enable) {
        ...
        log_error ("Watchdog is disabled in bolt and not started!");

    rv = test_loop(wd_enable);
