...
sub get_tgt($)
{
	...
	if (($chip eq "7145a0") || ($chip eq "7145b0")){
		$rg_defaults = "$RGDIR/targets/93385MWVG/93385MWVG";
	}
	elsif ($chip =~ /3390/) {
		if ($suffix =~ /dcm/) {
...
sub cmd_defaults($)
{
	...
	if(($chip eq "7145a0") || ($chip eq "7145b0") || ($chip =~ /3390/)) {
		$vendor{"CONFIG_USER_BRCM_BOOT_ASSIST_LITE"} = "n";
		...
		$vendor{"CONFIG_USER_BRCM_HCS32"} = "y";
		if(defined($linux{"CONFIG_BCM7145A0"})) {
		    $vendor{"CONFIG_USER_BRCM_BOOT_ASSIST_A0"} = "y";
		}
	    } else {
		$vendor{"CONFIG_USER_BRCM_BOOT_ASSIST_A0"} = "n";
	    ...
	    if(($chip eq "7145a0") || ($chip eq "7145b0")) {
	        $linux{"CONFIG_BCM_PININIT"} = "n";
	        $linux{"CONFIG_ATA"} = "y";
	        $linux{"CONFIG_SATA_AHCI_PLATFORM"} = "y";
	        $linux{"CONFIG_SATA_BRCMSTB"} = "y";
	        $vendor{"CONFIG_USER_BRCM_WPSCTL"} = "y";
	        $vendor{"CONFIG_USER_BRCM_DT"} = "n";
	    } else {
	        if($chip =~ /3390/) {
	            #Don't allow this for 3390! Code has hard coded 7145 reg!
	...
	while(defined($suffix) && ($suffix ne "")) {
		...
		if($mod eq "small") {
			...
			read_cfg("defaults/override.linux-dcm", \%linux_o);
			override_cfg(\%linux, \%linux_o);
		} elsif($mod eq "nocms") {
			if(-d $RGDIR) {
				print "mod='$mod'\n";
